...
class TestFunctions():
    ...
    def func_1(self):
        def f(x):
            return tf.pow(x[0],2) + tf.pow(x[1],2) + tf.pow(x[2],2) + tf.pow(x[3],2) + tf.pow(x[4],2) + tf.pow(x[5],2) + tf.pow(x[6],2) + tf.pow(x[7],2)
    ...
    def func_2(self):
        def f(x):
            return tf.pow(x[0],4) + 4 * tf.pow(x[0],3) * x[1] + 6 * tf.pow(x[0],2) + tf.pow(x[1],2) + 4 * x[0] * tf.pow(x[1],3) + tf.pow(x[1],4)
    ...
    def func_3(self):
        def f(x):
            return tf.pow(x[0] - 100, 2) + tf.pow(x[1] + 3, 2) + 5 * tf.pow(x[2] + 10, 2)
    ...
    def func_4(self):
        def f(t):
            def x1(t):
                return t
            def x2(t):
                return tf.pow(tf.math.sin(t), 2)
            def x3(t):
                return tf.pow(tf.math.cos(t), 2)
            def x4(t):
                return tf.pow(t, 2)
            def x5(t):
                return 2 * t - 1
    ...
    def func_5(self):
        def f(t):
            ...
            def x5(t):
                return t - 2
            def x6(t):
                return tf.pow(tf.math.sin(t), 4)
            def x7(t):
                return tf.pow(tf.math.cos(t), 4)
            return tf.pow(x1(t[0]), 2) + x2(t[0]) + 3 * x3(t[0]) + x4(t[0]) + tf.pow(x5(t[1]), 2) + x6(t[1]) + x7(t[1])
    ...
    def func_6(self):
        def f(x):
            return tf.pow(x[0] - 1, 2) + tf.pow(x[1], 2) + x[2] + 2 * x[3] + tf.pow(x[4], 3) + x[5]
